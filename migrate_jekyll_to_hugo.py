...
================================
Applies in-place fixes to branch 1 of precice.github.io

Categories handled:
  1. Frontmatter: permalink → url
  2. Frontmatter: layout references
  3. Jekyll include tags → Hugo partial stubs
  4. Liquid for/if/unless/assign tags in content (.md) files → Hugo equivalents
  5. data/ YAML files — SKIPPED (already correct for Hugo)

Usage:
    python migrate_jekyll_to_hugo.py [--repo /path/to/repo] [--dry-run]

Options:
    --repo      Path to the repo root (default: current directory)
    --dry-run   Print what would change, but don't write files
    --verbose   Print every change made
"""
...
# data/ directory files — skip (already correct for Hugo)
DATA_SKIP_PREFIXES = ("data/", "_data/")

# Layout name mappings: Jekyll → Hugo
LAYOUT_MAP = {
    "default":        "baseof",
    "page":           "page",
    "post":           "single",
...
def should_skip(path: Path, repo_root: Path) -> bool:
    ...
    for part in parts:
        if part in SKIP_DIRS or part.startswith("."):
            # But allow .github YAML issue templates — don't skip those
            if part == ".github":
                return False
            if part.startswith("."):
                return True
...
# ─────────────────────────────────────────────────────────────────────────────
# TRANSFORMATION 1: Frontmatter permalink → url
# ─────────────────────────────────────────────────────────────────────────────

def fix_permalink(lines, filepath):
    """
    Converts:   permalink: some-page.html
    To:         url: /some-page/

    Only fires inside the YAML frontmatter block (between --- delimiters).
    """
    result = []
    in_front = False
    front_count = 0
    ...
    for i, line in enumerate(lines):
        ...
        if in_front:
            ...
            if m:
                ...
                # Strip leading slash and .html, wrap in /slug/
                value = value.lstrip("/")
...
# ─────────────────────────────────────────────────────────────────────────────
# TRANSFORMATION 2: Frontmatter layout references
# ─────────────────────────────────────────────────────────────────────────────

def fix_layout_frontmatter(lines, filepath):
    """
    Converts:   layout: default
    To:         layout: baseof    (using LAYOUT_MAP)

    Only fires inside YAML frontmatter.
    Adds a comment so the developer knows to create the Hugo layout file.
    """
    result = []
    in_front = False
    front_count = 0
...
# ─────────────────────────────────────────────────────────────────────────────
# TRANSFORMATION 3: Jekyll {% include x.html %} → {{ partial "x.html" . }}
# ─────────────────────────────────────────────────────────────────────────────
...
def _include_replacer(m):
    raw = m.group(1).strip()
    # Split off any parameters like: warning.html content="foo"
    parts = raw.split(None, 1)
    filename = parts[0]
    params = parts[1] if len(parts) > 1 else None

    if params:
        # Convert Jekyll include params to Hugo partial dict
        # e.g. warning.html content="bar" → {{ partial "warning.html" (dict "content" "bar" "page" .) }}
        param_str = params.strip()
        # Parse key=value pairs
        kv_pairs = re.findall(r'(\w+)=["\']?([^"\'%}\s]+)["\']?', param_str)
        if kv_pairs:
...
def fix_include_tags(lines, filepath):
    """
    Converts {% include file.html %} → {{- partial "file.html" . -}}
    Works in both .html template files and .md content files.
    """
    result = []
    changed = False
...
# ─────────────────────────────────────────────────────────────────────────────
# TRANSFORMATION 4: Liquid tags in content/template files
# ─────────────────────────────────────────────────────────────────────────────

# --- for loops ---
FOR_RE = re.compile(
    r'\{%-?\s*for\s+(\w+)\s+in\s+([^\s%}]+(?:\.[^\s%}]+)*)\s*(?:limit:\d+)?\s*-?%\}'
)
...
def _convert_condition(cond):
    """Convert Jekyll condition to Hugo condition string."""
    cond = cond.strip()
    # page.X → .Params.X or special cases
    ...
    # == true / == false
    # Handle these BEFORE the unless wrapper adds 'not', to avoid double negatives
    # "X == false" in an unless → simplify to just "X" (unless not-false = if true)
    cond = re.sub(r'==\s*true', '', cond)
    cond = re.sub(r'!=\s*false', '', cond)
    # "X == false" → "not X" (will be used directly in if conditions)
    cond = re.sub(r'(\S+)\s*==\s*false', r'not \1', cond)
    # contains: Jekyll "collection contains item" → Hugo "in collection item"
    # e.g. "projectTags contains tag"  → "in $projectTags $tag"
    # e.g. "folder.output contains \"pdf\"" → "in $folder.output \"pdf\""
    def _contains_replacer(m):
        collection = m.group(1)
        item = m.group(2)
        def _hugo_var(v):
            # Already a Hugo var, dot expression, or quoted string literal
            if v.startswith('$') or v.startswith('.') or v.startswith('"') or v.startswith("'"):
                return v
            # Plain identifier — local loop variable, add $ prefix
            return f'${v}'
...
def _convert_for_collection(collection):
    """Convert Jekyll collection name to Hugo equivalent."""
    ...
    if m:
        if m.group(2):
            return f'.Site.Data.{m.group(1)}.{m.group(2)}'
        return f'.Site.Data.{m.group(1)}'
    # site.data[page.X]
    m = re.match(r'site\.data\[page\.(\w+)\](?:\.(\w+))?', collection)
    if m:
        if m.group(2):
            return f'(index .Site.Data (.Params.{m.group(1)})).{m.group(2)}'
        return f'(index .Site.Data .Params.{m.group(1)})'
    # page.X
...
def fix_liquid_tags(lines, filepath):
    """
    Converts Liquid/Jekyll template syntax to Hugo/Go template syntax.
    This is a best-effort conversion — complex logic gets a TODO comment.
    """
    result = []
    changed = False
    in_raw = False
    ...
    for i, line in enumerate(lines):
        ...
        # Handle raw blocks — pass through unchanged
        ...
        def for_replacer(m):
            ...
            # Use $loop only if the original loop body likely uses forloop.*
            # We always emit it so forloop.first/last refs can be fixed below
        ...
        def unless_replacer(m):
            ...
            # If condition already starts with 'not ', double negative — simplify to plain if
        ...
        # --- forloop.* inline (outside of if conditions, e.g. in HTML attributes) ---
        ...
        # page.X → .Params.X (catch-all, after specific ones)
...
def _jekyll_date_to_go(fmt):
    """Convert Jekyll strftime date format to Go time format."""
    mapping = {
        '%Y': '2006', '%y': '06',
        '%m': '01', '%-m': '1',
        '%d': '02', '%-d': '2',
...
# ─────────────────────────────────────────────────────────────────────────────
# TRANSFORMATION 5: categories → tags (frontmatter)
# ─────────────────────────────────────────────────────────────────────────────

def fix_categories(lines, filepath):
    """
    Hugo uses tags and categories both, but Jekyll 'categories' field
    needs taxonomy configured. Add a note comment.
    """
    result = []
    in_front = False
    front_count = 0
...
# ─────────────────────────────────────────────────────────────────────────────
# TRANSFORMATION 6: published: false → draft: true
# ─────────────────────────────────────────────────────────────────────────────
...
def process_file(filepath: Path, repo_root: Path, dry_run: bool, verbose: bool):
    ...
    # Skip the report and the script itself — don't process our own output
...
def write_report(repo_root: Path, dry_run: bool):
    ...
    for kind, items in sorted(by_type.items()):
        ...
        lines.append("|------|------|--------|-------|\n")
        for item in items[:50]:  # cap at 50 per type for readability
...
def main():
    parser = argparse.ArgumentParser(
        description="Jekyll → Hugo in-place migration script for precice.github.io branch 1"
    )
    parser.add_argument(
        "--repo",
        default=".",
